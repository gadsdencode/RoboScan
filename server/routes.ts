...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Get user's purchased premium fields
  app.get("/api/llms-fields/purchases", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
  ...
  // Create payment intent for premium field
  app.post("/api/llms-fields/create-payment-intent", isAuthenticated, async (req: any, res) => {
    try {
      const { fieldKey } = z.object({
  ...
  // Confirm premium field payment
  app.post("/api/llms-fields/confirm-payment", isAuthenticated, async (req: any, res) => {
    try {
      const { paymentIntentId } = z.object({
  ...
  // ============== Robots.txt Premium Fields Routes ==============
  
  // Get user's purchased premium robots fields
  app.get("/api/robots-fields/purchases", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
  ...
  app.post("/api/robots-fields/create-payment-intent", isAuthenticated, async (req: any, res) => {
    try {
      const { fieldKey } = z.object({
        fieldKey: z.string().min(1, "Field key is required"),
      }).parse(req.body);
  ...
  app.post("/api/robots-fields/confirm-payment", isAuthenticated, async (req: any, res) => {
    try {
      const { paymentIntentId } = z.object({
        paymentIntentId: z.string().min(1, "Payment intent ID is required"),
      }).parse(req.body);
